...
#
# Copyright (c) 2008-2010 by Pete Goodeve
#
# vers 2010/08/29 -- Ruby 1.9
#
#############################################
...
class MidiTrack
	...
	def read_system_or_meta
		...
		ev.length = length	# (excludes meta byte)
	 	@bytes_left -= length
	 	@command = 0	# maybe a litle protection from bad values...
		return ev
	end
  ...
  ## Output section
	...
	def add(ev)
		...
		if !ev.time ||
			...
			@evlist.each_with_index {|item, i|
				indx = i if item.time && item.time > ev.time}
# 			puts "inserting #{ev} at #{indx} -- before time #{@evlist[indx]}"
